...
class Show_Submissions_Settings {
    ...
    public function enqueue_admin_scripts($hook) {
        ...
        wp_enqueue_script(
            'show-submissions-settings',
            plugin_dir_url(dirname(__FILE__)) . 'dist/js/settings.min.js',
            array('jquery'),
            '1.0.0',
            true
        );
    ...
    public function update_checkbox_status() {
        ...
        if ($type === 'approval' || $type === 'new_event') {
            $current_values = get_option($option_name, array());
    ...
    public function render_promoters_section() {
        ...
        foreach ($organizers as $organizer) {
            ...
            echo '<li>';
            echo sprintf(
                '<label><input type="checkbox" class="status-checkbox" data-type="promoter" data-id="%d" name="show_submissions_promoter_status[%d]" value="1" %s> %s</label>',
                $organizer->ID,
                $organizer->ID,
                $checked,
    ...
    public function render_venues_section() {
        ...
        foreach ($venues as $venue) {
            ...
            echo '<li>';
            echo sprintf(
                '<label><input type="checkbox" class="status-checkbox" data-type="venue" data-id="%d" name="show_submissions_venue_status[%d]" value="1" %s> %s</label>',
                $venue->ID,
                $venue->ID,
                $checked,
    ...
    public function render_new_event_notification_field() {
        $settings = get_option('show_submissions_new_event_notification', array(
            'enabled' => false,
            'to' => '',
            'cc' => '',
            'bcc' => ''
    ...
    public function render_approval_notification_field() {
        $settings = get_option('show_submissions_approval_notification', array(
            'enabled' => false,
            'to' => '',
            'cc' => '',
            'bcc' => ''
