...
if ( ! defined( 'SHOW_SUBMISSIONS_WP_STUBS_LOADED' ) ) {
	...
	if ( ! function_exists( 'selected' ) ) {
		function selected( $selected, $current, $should_echo = true ) {
	...
	if ( ! function_exists( 'checked' ) ) {
		function checked( $checked, $current = true, $should_echo = true ) {
			$is_checked = (string) $checked === (string) $current || ( ! empty( $checked ) && true === $current );
	...
	if ( ! function_exists( 'wp_register_style' ) ) {
		function wp_register_style( $handle, $src = false, $deps = array(), $ver = false, $media = 'all' ) {
			/* no-op */ } }
	...
	if ( ! function_exists( 'wp_nonce_field' ) ) {
		function wp_nonce_field( $action = -1, $name = '_wpnonce' ) {
			echo '<input type="hidden" name="' . esc_attr( $name ) . '" value="stubbed-nonce" />';
		}
	}
	...
	if ( ! function_exists( 'wp_create_nonce' ) ) {
		function wp_create_nonce( $action = -1 ) {
			return 'stubbed-nonce'; } }
	if ( ! function_exists( 'check_ajax_referer' ) ) {
		function check_ajax_referer( $action = -1, $query_arg = '_ajax_nonce', $should_die = true ) {
			return true; } }
	...
	if ( ! function_exists( 'wp_check_filetype' ) ) {
		function wp_check_filetype( $filename ) {
			...
			$type = 'png' === $ext ? 'image/png' : ( 'gif' === $ext ? 'image/gif' : 'image/jpeg' );
			return array(
				'ext'  => $ext,
				'type' => $type,
			);
	...
	if ( ! function_exists( 'wp_generate_attachment_metadata' ) ) {
		function wp_generate_attachment_metadata( $attach_id, $file ) {
	...
	if ( ! function_exists( 'wp_tempnam' ) ) {
		function wp_tempnam( $filename = '', $dir = '' ) {
			return tempnam( sys_get_temp_dir(), 'wp-' ); } }
	...
	// Database stub ($wpdb)
	require_once __DIR__ . '/class-wpdb-stub.php';

	// dbDelta stub used by activator
	if ( ! function_exists( 'db_delta' ) ) {
