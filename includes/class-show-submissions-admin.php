...
class Show_Submissions_Admin {
	...
	public function render_submission_details() {
		if ( ! isset( $_GET['id'] ) || ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( $_GET['_wpnonce'], 'view_submission_details_' . $_GET['id'] ) ) {
	...
	private function save_submission_details() {
		...
		if ( $existing && isset( $existing->status ) && 'Approved' === $existing->status && 'Approved' === $posted_status ) {
			$data = array(
				'status'   => 'Approved',
				'approved' => 1,
			);
			...
			$data = array(
				...
				'venue_address'    => \trim( sanitize_textarea_field( $_POST['venue_address'] ?? '' ) ),
			...
			if ( 1 === $data['approved'] || 'Approved' === $data['status'] ) {
				$data['approved'] = 1;
		...
		$now_approved = ( isset( $data['approved'] ) && 1 === \intval( $data['approved'] ) ) ? 1 : 0;
		if ( $existing && 0 === \intval( $existing->approved ) && 1 === $now_approved ) {
			...
			if ( $submission ) {
				...
				$event_data = array(
					...
					'met-input'   => array(
						'_EventStartDate'  => \trim( $event_date . ' ' . $start_time ),
						'_EventEndDate'    => \trim( $event_date . ' ' . \gmdate( 'H:i:s', \strtotime( $start_time . ' +3 hours' ) ) ),
	...
	public function enqueue_admin_assets( $hook ) {
		if ( ! \in_array( $hook, array( 'toplevel_page_show-submissions', 'show-submissions_page_show-submission-details' ), true ) ) {
			return;
		}
			...
			<table class="wp-list-table widefat fixed striped">
				...
				<tbody>
					...
					foreach ( $submissions as $submission ) :
						...
						$view_details_url = wp_nonce_url( admin_url( 'admin.php?page=show-submission-details&id=' . $submission->id ), 'view_submission_details_' . $submission->id );
						?>
						<tr data-id="<?php echo esc_attr( $submission->id ); ?>">
							...
							<td><?php echo esc_html( $this->get_venue_display_name( $submission->venue_name ) ); ?></td>
		...
		if ( $approved ) {
			...
			$event_data = array(
				...
				'met-input'    => array(
					...
					'_EventEndDate'    => $event_date . ' ' . gmdate( 'H:i:s', strtotime( $start_time . ' +3 hours' ) ),
